#include "GuestGameRoom.h"
#include <QLabel>

GuestGameRoom::GuestGameRoom(const std::string &roomCode, QWidget *parent)
    : BaseMenu(parent)
{
    ...
    QLabel *pinDisplay = new QLabel(QString::fromStdString(roomCode), central);
    ...
    playersLabel_ = new QLabel("Waiting for players...", central);
...
void GuestGameRoom::updatePlayers(const std::vector<std::string> &players)
{
    if (players.empty()) {
        playersLabel_->setText("Waiting for players...");
        return;
    }
